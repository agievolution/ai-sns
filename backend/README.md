...
|----------|--------------|
| `get_ai_config()` | `backend.config.settings.Settings._load_ai_config()` |
| `ConnectionManager` | `backend.shared.websocket_manager.ConnectionManager` |
| OpenAI API calls | `backend.shared.ai_client.AIClient` |
| Database session | `backend.config.database.get_db()` |
| Utility functions | `backend.shared.utils` |
