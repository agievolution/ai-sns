...
async def start_social_engine(db: Session = Depends(get_db)):
...
async def stop_social_engine(db: Session = Depends(get_db)):
...
async def get_ai_chat_config(user_id: str = None, db: Session = Depends(get_db)):
    """Get AI chat configuration"""
    service = SNSService(db)
    return service.get_ai_chat_config(user_id)
...
async def update_ai_chat_config(
    ...
    service = SNSService(db)
    return service.update_ai_chat_config(user_id, request.dict(exclude_unset=True))
...
async def get_social_roles(db: Session = Depends(get_db)):
    """Get social roles (prompts with SNS tag)"""
    service = SNSService(db)
    return service.get_social_roles()
...
async def update_user_info(
    ...
    service = SNSService(db)
    return service.update_user_info(request)
...
async def update_map_config(
    ...
    service = SNSService(db)
    return service.update_map_config(request)
