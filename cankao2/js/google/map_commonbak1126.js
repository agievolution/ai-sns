...
// 使用 async/await 处理异步加载，确保模型完全加载后再执行后续操作
    ...
    const loadModel = async () => {
        try {
            // 同时加载女孩和男孩模型
            const [gltfGirl, gltfBoy] = await Promise.all([
                loadModelWithRetry(loader, 't-shirtgirl2.glb'),
                loadModelWithRetry(loader, 't-shirtboy2.glb')
            ]);

            // ===== 处理女孩模型 (tshirtgirl_0_0_0_0_1_0.glb) =====
            model = gltfGirl.scene;
            // 添加环境光
            const ambientLight = new THREE.AmbientLight(0xffffff, 0.75);
            //overlay.scene.add(ambientLight);
            // 添加平行光
            ...
            // 查找场景中的所有网格
            const modelMeshes_found = findMeshes(gltfGirl.scene);
            // 将找到的网格添加到全局数组
            ...
            // 创建动画混合器并播放动画
            if (gltfGirl.animations && gltfGirl.animations.length > 0) {
                const mixer = new THREE.AnimationMixer(gltfGirl.scene);
                const action = mixer.clipAction(gltfGirl.animations[0]);
                mixer.timeScale = 0.5;
            ...
            // ===== 处理男孩模型 - 克隆女孩的骨骼和动画 =====
            // 复制女孩模型（含骨骼）
            const boyModelClone = THREE.SkeletonUtils.clone(gltfGirl.scene);

            // 先设置克隆模型的位置（在替换网格之前）
            boyModelClone.position.set(130, 0, -250);

            // 用男孩模型的网格替换克隆模型的网格材质和几何体
            boyModelClone.traverse(child => {
                if (child.isMesh) {
                    const meshBoy = gltfBoy.scene.getObjectByName(child.name);
                    if (meshBoy) {
                        child.geometry = meshBoy.geometry;
                        child.material = meshBoy.material;
                    }
                    // 给模型添加点击事件
                    child.cursor = 'pointer';
                    child.userData.isClickable = true;
            ...
            // 将男孩模型添加到场景中
            overlay.scene.add(boyModelClone);

            // 查找场景中的所有网格
            const boyModelMeshes_found = findMeshes(boyModelClone);
            // 将找到的网格添加到全局数组
            all_model_meshes.push(...boyModelMeshes_found);

            // 创建动画混合器并播放动画（使用女孩的动画）
            if (gltfGirl.animations && gltfGirl.animations.length > 0) {
                const mixerBoy = new THREE.AnimationMixer(boyModelClone);
                const actionBoy = mixerBoy.clipAction(gltfGirl.animations[0]);
                mixerBoy.timeScale = 0.5;
                actionBoy.setDuration(1).play();
                mixers.push(mixerBoy);
                console.log("男孩模型动画已启动（克隆女孩动画）");
            }
            console.log("男孩模型加载成功（克隆女孩骨骼）");
...
// 调用 loadModel 函数
    ...
    // === 已注释：原来的 loadModel2 函数已被整合到 loadModel 中，使用克隆动画方式 ===
