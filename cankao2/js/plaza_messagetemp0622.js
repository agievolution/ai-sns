// Plaza Message System - Optimized Version
...
// 动态加载消息并获取容器的边界矩形
const messages = messageData.map(messageItem => {
    // 创建一个div元素来显示每条消息
...
function play() {
    ...
    setTimeout(play, SLIDE_IN_DURATION + WAIT_BEFORE_NEXT);
}
...
/**
 * 停止消息轮播动画
 * 该函数会立即停止所有正在进行的动画和定时器，并重置所有消息元素的状态
 */
function stop() {
    // 清除所有定时器 - 通过清除一个范围内的定时器ID来确保停止所有相关定时器
